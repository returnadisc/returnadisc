...
=======
            ...
            # Kolla om det finns pending QR
            pending_qr = session.pop('pending_qr_activation', None)
            if pending_qr:
                flash('Välkommen! Aktivera nu din QR-kod.', 'success')
                return redirect(url_for('disc.activate_qr_page', qr_id=pending_qr))
            
            flash('Välkommen! Aktivera din QR-kod för att komma igång.', 'success')
...
=======
        ...
        pending_qr = session.pop('pending_qr_activation', None)
        if pending_qr:
            flash('Du är inloggad! Aktivera nu din QR-kod.', 'success')
            return redirect(url_for('disc.activate_qr_page', qr_id=pending_qr))
        
        # Kolla om har QR
        user_qr = db.get_user_qr(user['id'])
        if not user_qr:
            flash('Välkommen! Aktivera din QR-kod för att komma igång.', 'success')
            return redirect(url_for('disc.activate_qr_page'))
